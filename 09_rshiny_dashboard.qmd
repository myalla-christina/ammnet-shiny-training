...
**Goal:** Build the layout before touching any data 
or reactivity. By the end your app runs and looks 
like a dashboard — with no data in it yet.
...
Create a new file called `app.R` in your 
project root. Copy this exactly:
...
Save as `images/stage1_running.png`.
:::

### What just happened

You have a running Shiny app with zero data 
and zero reactivity. The server is completely 
empty. This confirms the layout works before 
we add any complexity.
...
**1a.** Change the `height` of `layout_columns()` 
wrapping the value boxes from `"150px"` to 
`"200px"`. What changes?

**1b.** Add a fourth `value_box()` with
`title = "Total estimated cases"`, 
`value = "—"`, `bs_icon("people")`, 
`theme = "info"`.

**1c.** Add `col_widths = c(7, 5)` to the 
`layout_columns()` wrapping the map and bar 
chart. Which card gets wider?

::: {.callout-caution collapse="true"}
## Show solution

**1a.** The value box row becomes taller — 
more breathing room for the KPI values.
...
**Goal:** Load real data, add sidebar inputs, 
and make the three value boxes reactive. 
By the end changing the year dropdown updates 
all three KPI numbers instantly.
...
Run the app — inputs appear but nothing 
reacts yet. That changes in the next step.

![Stage 2 sidebar with inputs](images/stage2_sidebar.png)

::: callout-tip
## Snapshot
Take a screenshot of the sidebar showing 
all inputs. Save as `images/stage2_sidebar.png`.
:::

### Step 3 — Connect value boxes to data

Replace the three hardcoded `value = "—"` 
with `textOutput()` placeholders:
...
- Moving the threshold slider updates 
  the high burden count
...
::: callout-tip
## Snapshot
Take a screenshot showing the three value 
boxes with real numbers. Save as 
`images/stage2_complete.png`.
:::
...
One filter, three outputs. Faster and 
easier to maintain.
:::

### Exercise 2 {.unnumbered}

**2a.** Add a fourth value box showing the 
name of the highest burden district. 
Use `bs_icon("geo-alt")` and `theme = "info"`.

**2b.** Add a `textOutput("sidebar_summary")` 
below the slider in the sidebar. In the server 
add a `renderText()` that shows how many 
districts are currently selected.

**2c.** Move the threshold slider and observe 
`kpi_count` changing. What happens at 
threshold = 0? What about 500?
...
**2c.** At threshold = 0 all districts are 
high burden. At 500 none are — `kpi_count` 
shows 0.
:::
...
- [ ] District dropdown updates when 
      region changes
...
**Goal:** Replace both card placeholders with 
real interactive visualisations. By the end 
you have a choropleth map and a ranked bar 
...
Add these below your data loading, 
before the UI:
...
`make_pal()` is called inside `renderLeaflet()`. 
`top_districts()` is called inside both 
`renderPlotly()` and potentially `renderDT()`. 
Writing them once outside the server keeps 
the server clean and avoids repetition.
:::

### Step 2 — Update the UI

Replace both placeholder cards with output 
containers:
...
1. Map with a district hovered — 
   `images/stage3_map_hover.png`
2. Bar chart with a bar hovered — 
   `images/stage3_bar_hover.png`
:::

### Exercise 3 {.unnumbered}

**3a.** Change the map background tiles from 
`CartoDB.Positron` to `Esri.WorldGrayCanvas`. 
Run `names(providers)` to explore options.

**3b.** Add ITN coverage as a second colour 
option. Add a `radioButtons()` to the sidebar:
...
Then update `renderLeaflet()` to use 
`input$map_indicator` to select which 
column to colour by.

**3c.** Add `col_widths = c(6, 6)` to the 
`layout_columns()` — confirm both cards 
are equal width.
...
**Goal:** Complete the dashboard with the 
rainfall-incidence trend chart and make it 
look professional with theming and polish.
...
::: callout-tip
## Snapshot
Take a screenshot showing the seasonal 
pattern clearly — rainfall bars and 
incidence lines peaking at different months. 
Save as `images/stage4_trend.png`.
:::
...
Click **Run App**. You now have a complete 
professional malaria burden dashboard:
...
::: callout-tip
## Final snapshot
Take a full screenshot of the complete 
dashboard. Save as `images/stage4_final.png`.
...
**4a.** Try three different Bootswatch themes — 
`"cosmo"`, `"minty"`, `"sandstone"`. 
Which looks best with the malaria colour palette?

**4b.** Add a `selectInput()` to control 
how many districts appear in the bar chart:
...
**4c.** Add a `DT::dataTableOutput("data_table")` 
in a new card below the trend chart. 
In the server add `renderDT()` showing 
district, region, incidence and ITN coverage 
for the filtered data sorted by incidence.
...
- [ ] Trend chart shows rainfall bars 
      and incidence lines
- [ ] Rainfall peaks July–August
- [ ] Incidence peaks September–October
...
Your dashboard is ready to share publicly 
via **shinyapps.io** — free hosting for 
Shiny apps.
...
Your token and secret are on your 
shinyapps.io dashboard under 
`Account → Tokens`.
...
Sufficient for training. For production 
use consider Posit Connect.
:::
...
You have built a complete interactive malaria 
burden dashboard from scratch in two hours.
...
**Scale your code** — as your app grows, 
split `app.R` into separate files:
...
::: callout-tip
## Keep building
Adapt this dashboard to your own country 
or programme data. The patterns you learned 
today — reactive(), renderLeaflet(), 
renderPlotly() — are the same ones used 
in production dashboards at national 
